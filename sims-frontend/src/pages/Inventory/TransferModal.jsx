...
export default function TransferModal({ warehouses: propWH, isAdmin, userId, onClose, onSuccess }) {
  ...
  const schema = Yup.object({
    ...
    toWarehouseId:    Yup.number()
      ...
      .test('diff', 'Must differ from source', function (v) { return String(v) !== String(this.parent.fromWarehouseId); }),
  ...
  const formik = useFormik({
    initialValues: { product_id: '', fromWarehouseId: '', toWarehouseId: '', quantity: '', notes: '' },
  ...
  useEffect(() => {
    ...
    if (!pid || !wid) { setSourceQty(null); return; }
    inventoryAPI.getAll({ warehouseId: wid })
      .then((r) => {
        const item = (r.data.data?.inventory || []).find((i) => String(i.product_id) === String(pid));
        setSourceQty(item ? item.available_qty ?? item.quantity : 0);
      })
      .catch(() => setSourceQty(null));
  }, [F.values.product_id, F.values.fromWarehouseId]);

  const fromWH = myWarehouses.find((w) => String(w.warehouse_id) === String(F.values.fromWarehouseId));
        ...
        {!confirming ? (
          ...
          <form id="transfer-form" onSubmit={F.handleSubmit}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <Input label="Search Product" placeholder="Type to filter…" value={prodSearch} onChange={(e) => setProdSearch(e.target.value)} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'flex-end' }}>
                <Select label="From Warehouse" required value={F.values.fromWarehouseId}
                ...
                <Select label="To Warehouse" required value={F.values.toWarehouseId}
                  onChange={(e) => F.setFieldValue('toWarehouseId', e.target.value)}
                  error={err('toWarehouseId')}>
                  <option value="">Destination…</option>
                  {myWarehouses.filter((w) => String(w.warehouse_id) !== String(F.values.fromWarehouseId)).map((w) => (
              ...
              {sourceQty !== null && (
                <div style={{ background: 'var(--color-surface-alt)', borderRadius: 'var(--radius-md)', padding: '8px 12px', fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)', fontFamily: 'var(--font-sans)' }}>
                  Available in source warehouse: <strong style={{ color: sourceQty > 0 ? 'var(--color-success)' : 'var(--color-danger)' }}>{fmt(sourceQty)} units</strong>
                </div>
              )}

              <Input label="Quantity to Transfer" required type="number" min="1" max={sourceQty || undefined} {...F.getFieldProps('quantity')} error={err('quantity')} />
              <Input label="Notes" placeholder="Reason for transfer (optional)" {...F.getFieldProps('notes')} />
            </div>
          </form>
        ) : (
          /* ── Step 2: confirmation ── */
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, alignItems: 'center', textAlign: 'center', padding: '12px 0' }}>
            ...
            <h3 style={{ margin: 0, fontFamily: 'var(--font-sans)', color: 'var(--color-text-primary)' }}>Confirm Transfer</h3>

            <div style={{ background: 'var(--color-surface-alt)', borderRadius: 'var(--radius-md)', padding: 16, width: '100%', textAlign: 'left' }}>
              <dl style={{ margin: 0, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 24px' }}>
                ...
                <DLItem label="Quantity"    value={<Badge variant="primary">{fmt(F.values.quantity)} units</Badge>} />
      ...
      <Modal.Footer>
        {confirming ? (
          <>
            ...
            <Button variant="primary" size="sm" loading={F.isSubmitting} leftIcon={<CheckIcon style={{ fontSize: 16 }} />} onClick={F.submitForm}>
              Confirm Transfer
            </Button>
          </>
        ) : (
          <>
            ...
            <Button variant="primary" size="sm" onClick={() => { F.validateForm().then((e) => { if (Object.keys(e).length === 0) setConfirming(true); else F.submitForm(); }); }}>
              Review Transfer →
            </Button>
          </>
        )}
...
function DLItem({ label, value }) {
  return (
    <div>
      <dt style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'var(--font-sans)', marginBottom: 2 }}>{label}</dt>
      <dd style={{ margin: 0, fontSize: 'var(--text-base)', fontWeight: 500, color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)' }}>{value}</dd>
    </div>
  );
}
