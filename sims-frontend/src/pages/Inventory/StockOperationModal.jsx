            ...
            {mode !== 'adjust' && (
              <>
                ...
                <Select
                  label='Warehouse'
                  required
                  value={F.values.warehouse_id}
                  onChange={(e) => F.setFieldValue('warehouse_id', e.target.value)}
                  error={err('warehouse_id')}
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <Input
                    ...
                    {...F.getFieldProps('batch_no')}
                    error={err('batch_no')}
                  />
                  <Input
                    label='Expiry Date'
                    type='date'
                    {...F.getFieldProps('expiry_date')}
                    error={err('expiry_date')}
                  />
                </div>
                <Input
                  label='Storage Location'
                  placeholder='Rack A1, Zone B…'
                  {...F.getFieldProps('location')}
                />
            ...
            {mode === 'stock-out' && (
              <>
                <Input
                  label='Reason'
                  required
                  placeholder='Sales / Damage / Internal use…'
                  {...F.getFieldProps('reason')}
                  error={err('reason')}
                />
                <Input
                  label='Reference #'
                  placeholder='Sales order / Request # (optional)'
                  {...F.getFieldProps('reference_no')}
                />
              </>
            )}
            ...
            {mode === 'adjust' && (
              <>
                <Select
                  ...
                  required
                  value={F.values.inventory_id}
                  onChange={(e) => F.setFieldValue('inventory_id', e.target.value)}
                  error={err('inventory_id')}
                >
                ...
                <Input
                  ...
                  type='number'
                  min='0'
                  {...F.getFieldProps('new_qty')}
                  error={err('new_qty')}
                  helper='Set the corrected total quantity (not a delta)'
                />
                <Input
                  label='Reason'
                  required
                  placeholder='Physical count / Damage / System error…'
                  {...F.getFieldProps('reason')}
                  error={err('reason')}
                />
                <Input label='Notes' placeholder='Optional notes…' {...F.getFieldProps('notes')} />
