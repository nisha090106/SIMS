...
export default function InventoryList({ warehouseId: fixedWarehouseId } = {}) {
  ...
  const fetchInventory = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: 'OK',
        payload: { rows: d.inventory, total: d.total, page: d.page, totalPages: d.totalPages },
      });
    } catch (err) {
      dispatch({ type: 'ERR' });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {(isAdmin || isMgr) && (
            <>
              <Button
                variant='secondary'
                size='sm'
                leftIcon={<TransferIcon style={{ fontSize: 16 }} />}
              ...
              <Button
                variant='secondary'
                size='sm'
                leftIcon={<AdjustIcon style={{ fontSize: 16 }} />}
                onClick={() => setOpModal('adjust')}
              >
                Adjust
              </Button>
            </>
          ...
          <Button
            ...
            title={
              lastRefresh ? `Last updated: ${lastRefresh.toLocaleTimeString()}` : 'Click to refresh'
            }
          >
            Refresh
          </Button>
      ...
      <Card padding={false}>
        ...
        <Table
          ...
          emptyText={
            tab === 'low'
              ? 'No low stock items — all products are healthy.'
              : tab === 'out'
          ...
          renderRow={(row, i) => (
            ...
            >
              ...
              <td style={{ ...td, color: 'var(--color-text-muted)', fontSize: 'var(--text-xs)' }}>
                {row.location || '—'}
              </td>
              <td
                style={{
              ...
              <td style={{ ...td, textAlign: 'center' }}>
                <Badge variant={row.available_qty > 0 ? 'success' : 'danger'} size='sm'>
