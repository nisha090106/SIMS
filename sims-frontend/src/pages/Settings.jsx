          ...
          <Card key={i}>
            <div style={{ padding: 20 }}>
              {[1, 2, 3].map((j) => (
                <div
                  key={j}
                  style={{ height: 38, borderRadius: 8, background: '#f1f5f9', marginBottom: 16 }}
                />
              ))}
            </div>
          </Card>
            ...
            {({ values, errors, touched, handleChange, handleBlur, isSubmitting }) => (
              <Form style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div>
                  ...
                  <Badge
                    variant={ROLE_VARIANT[profile?.role] || 'neutral'}
                    size='md'
          ...
          {[
            {
              ...
              value: (
                <Badge variant={STATUS_VARIANT[profile?.status] || 'neutral'}>
                  {profile?.status}
                </Badge>
              ),
            },
            ...
            { label: 'Last Updated', value: fmtDate(profile?.updated_at) },
            {
              label: 'User ID',
              value: (
                <code style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>#{profile?.id}</code>
              ),
            },
          ].map((item, i) => (
            <div
        ...
        <Table
          ...
          renderRow={(u, i) => (
            ...
            >
              ...
              <td style={{ ...td, fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)' }}>
                {u.email}
              </td>
              ...
              <td style={{ ...td, textAlign: 'center' }}>
                <Badge variant={STATUS_VARIANT[u.status] || 'neutral'} size='sm' dot>
                  {u.status}
                </Badge>
              </td>
              <td style={{ ...td, fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)' }}>
                {fmtDate(u.created_at)}
              </td>
              <td style={{ ...td, textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                  ...
                  <Button
                    variant='ghost'
                    size='sm'
                    leftIcon={<KeyIcon style={{ fontSize: 14 }} />}
                  ...
                  {Number(u.id) !== Number(currentUser?.id) &&
                    (u.status === 'active' ? (
                      <Button
                        variant='ghost'
                        size='sm'
                        leftIcon={<BlockIcon style={{ fontSize: 14 }} />}
                      ...
                      <Button
                        variant='ghost'
                        size='sm'
                        leftIcon={<ActivateIcon style={{ fontSize: 14 }} />}
                        loading={actionLoading === `status-${u.id}`}
                        onClick={() => handleToggleStatus(u)}
            ...
            {({ values, errors, touched, handleChange, handleBlur, isSubmitting }) => (
              <Form>
                <Modal.Body style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  >
                    Set a new password for <strong>{resetTarget.email}</strong>. They will need to
      ...
      {deleteTarget && (
        ...
        >
          <Modal.Body>
            ...
            >
              Deactivate <strong>{deleteTarget.email}</strong>? They will no longer be able to log
              in. This can be undone by reactivating the account.
            </p>
          </Modal.Body>
          <Modal.Footer>
          ...
          renderRow={(log, i) => (
            <React.Fragment key={log.logId}>
              ...
              >
                ...
                <td style={td}>
                  <Badge
                    variant={ACTION_VARIANT[log.action?.toLowerCase()] || 'neutral'}
                    size='sm'
                ...
                <td style={{ ...td, textAlign: 'right' }}>
                  {log.changes && (
                    <Button
                      variant='ghost'
                      size='sm'
                      onClick={() => setExpanded(expanded === log.logId ? null : log.logId)}
                    >
...
export default function Settings() {
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 0, maxWidth: 1200, margin: '0 auto' }}
    >
      {/* Page header */}
