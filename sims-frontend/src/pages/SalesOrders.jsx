...
import {
  ...
  Stepper, Step, StepLabel, InputAdornment, Alert,
  CircularProgress, Divider, Drawer, Stack, Snackbar,
...
function CreateOrderModal({ open, onClose, onSuccess, warehouses, userRole, userWarehouseIds }) {
  ...
  useEffect(() => {
    if (!warehouseId || items.length === 0) { setStockMap({}); return; }
    (async () => {
      try {
        const res = await inventoryAPI.getAll({ warehouseId, limit: 500 });
        const inv = res.data.data?.inventory || [];
        const map = {};
        inv.forEach((r) => { map[r.product_id] = r.available_qty ?? r.quantity ?? 0; });
        setStockMap(map);
      } catch { /* silent */ }
    })();
  ...
  const reset = () => {
    setStep(0); setCustomerName(''); setWarehouseId(''); setDeliveryDate('');
    setNotes(''); setItems([]); setStockMap({}); setProdSearch(''); setError('');
  };
  ...
  const allowedWarehouses = userRole === 'admin'
    ? warehouses
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth
      ...
      <Box sx={{ px: 3, pt: 1, pb: 0.5 }}>
        <Stepper activeStep={step} alternativeLabel>
          {STEPS.map((s) => (
            <Step key={s}><StepLabel sx={{ '& .MuiStepLabel-label': { fontSize: 12, fontWeight: 600 } }}>{s}</StepLabel></Step>
          ))}
        </Stepper>
      </Box>
      ...
      <DialogContent sx={{ pt: 2 }}>
        ...
        {step === 0 && (
          <Stack spacing={2}>
            <TextField label="Customer Name" required fullWidth size="small"
              value={customerName} onChange={(e) => setCustomerName(e.target.value)}
              placeholder="Enter customer name" />
            <TextField select label="Warehouse" required fullWidth size="small"
            ...
            <TextField label="Expected Delivery Date" type="date" fullWidth size="small"
        ...
        {step === 1 && (
          <Stack spacing={2}>
            ...
            <Autocomplete
              ...
              onChange={(_, product) => { addProduct(product); setProdSearch(''); }}
            ...
            {items.length === 0 ? (
              <Box sx={{ textAlign: 'center', py: 4, color: 'text.secondary' }}>
                ...
                <Typography variant="body2">No items added yet. Search for products above.</Typography>
              </Box>
            ) : (
                        ...
                        <TableRow key={idx} sx={{ '&:last-child td': { borderBottom: 0 } }}>
                          ...
                          <TableCell>
                            {avail === null ? (
                              ...
                              <Typography variant="caption" fontWeight={600} color={avail === 0 ? 'error' : avail < 10 ? 'warning.main' : 'success.main'}>
                                {avail} units
                              </Typography>
                            )}
                          </TableCell>
                          ...
                          <TableCell sx={{ width: 130 }}>
                            <TextField type="number" size="small" value={item.unit_price}
            ...
            {stockWarnings.length > 0 && (
              <Alert severity="warning" icon={<WarnIcon />} sx={{ borderRadius: 2 }}>
                <strong>{stockWarnings.length} item(s)</strong> exceed available stock.
                You can still create the order as a draft, but fulfillment will fail unless stock is replenished.
              </Alert>
            )}
        ...
        {step === 2 && (
          <Stack spacing={2}>
            <Paper variant="outlined" sx={{ borderRadius: 2, p: 2 }}>
              ...
              <Stack spacing={1}>
                {[
                  ...
                  ['Warehouse', allowedWarehouses.find((w) => String(w.warehouse_id) === String(warehouseId))?.name || '—'],
            ...
            <Paper variant="outlined" sx={{ borderRadius: 2, overflow: 'hidden' }}>
              ...
              {items.map((item, idx) => (
                <Box key={idx} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 2, py: 1.25, borderBottom: idx < items.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                  <Box>
                    <Typography variant="body2" fontWeight={600}>{item.product_name}</Typography>
                    <Typography variant="caption" color="text.secondary">{item.quantity} × {fmt(item.unit_price)}</Typography>
                  </Box>
            ...
            {stockWarnings.length > 0 && (
              <Alert severity="warning" sx={{ borderRadius: 2 }}>
                <strong>Stock warning:</strong> {stockWarnings.map((i) => i.product_name).join(', ')} exceed available stock.
                Order will be created as <strong>draft</strong> — fulfill after restocking.
              </Alert>
            )}
          </Stack>
        )}
      ...
      <DialogActions sx={{ px: 3, py: 2, borderTop: '1px solid #E2E8F0', gap: 1 }}>
        <Button onClick={handleClose} variant="outlined" size="small" sx={{ borderRadius: 2 }}>Cancel</Button>
        {step > 0 && (
          <Button onClick={() => setStep((s) => s - 1)} variant="outlined" size="small" sx={{ borderRadius: 2 }}>Back</Button>
        )}
        {step < 2 ? (
          <Button
            onClick={() => setStep((s) => s + 1)}
            disabled={(step === 0 && !canNext0) || (step === 1 && !canNext1)}
            variant="contained" size="small" sx={{ borderRadius: 2, minWidth: 80 }}>
            Next
          </Button>
        ) : (
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth
      ...
      <DialogContent>
        <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
          Fulfilling this order will <strong>deduct stock</strong> from the warehouse for each item below.
      ...
      <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
        <Button onClick={onClose} variant="outlined" size="small" sx={{ borderRadius: 2 }}>Cancel</Button>
        <Button onClick={onConfirm} variant="contained" color="success" size="small" disabled={loading}
      ...
      <DialogContent>
        {isDispatched && (
          <Alert severity="warning" sx={{ mb: 2, borderRadius: 2 }}>
            This order has already been <strong>dispatched</strong>. Cancelling will <strong>restore stock</strong> to the warehouse.
          </Alert>
        )}
        <Typography variant="body2" sx={{ mb: 2 }}>
          Cancel order <strong>{order.order_number}</strong> for <strong>{order.customer_name}</strong>?
        </Typography>
        <TextField label="Reason (optional)" fullWidth size="small" multiline rows={2}
          value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Enter reason for cancellation…" />
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
        <Button onClick={onClose} variant="outlined" size="small" sx={{ borderRadius: 2 }}>Keep Order</Button>
        <Button onClick={() => onConfirm(reason)} variant="contained" color="error" size="small" disabled={loading}
...
function OrderDrawer({ order, open, onClose, onFulfill, onCancel, onDeliver, userRole }) {
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose}
      PaperProps={{ sx: { width: { xs: '100vw', sm: 480 }, bgcolor: '#FFFFFF', borderLeft: '1px solid #E2E8F0' } }}>
      {/* Header */}
      ...
      <Box sx={{ overflowY: 'auto', flex: 1 }}>
              ...
              {TIMELINE_STEPS.map((s, i) => (
                <React.Fragment key={s}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 64 }}>
                    ...
                    <Typography variant="caption" sx={{ mt: 0.5, fontSize: 10, color: i <= statusIdx ? '#1E293B' : '#94A3B8', fontWeight: i === statusIdx ? 700 : 400, textAlign: 'center' }}>
                      {STATUS[s]?.label || s}
                    </Typography>
                  </Box>
                  {i < TIMELINE_STEPS.length - 1 && (
                    <Box sx={{ flex: 1, height: 2, bgcolor: i < statusIdx ? '#3B82F6' : '#E2E8F0', mb: 2.5, transition: 'all 0.2s' }} />
                  )}
                </React.Fragment>
              ))}
            </Box>
        ...
        <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #F1F5F9' }}>
          ...
          <Stack spacing={1}>
            {[
              ...
              ['Created By',     order.created_by_user ? `${order.created_by_user.first_name} ${order.created_by_user.last_name}` : '—'],
        ...
        <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #F1F5F9' }}>
          ...
          <Stack spacing={0}>
            {items.map((item, i) => (
              <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.25, borderBottom: i < items.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                <Box>
                  <Typography variant="body2" fontWeight={600}>{item.product_name}</Typography>
                  <Typography variant="caption" color="text.secondary">{item.quantity} × {fmt(item.unit_price)} · SKU: {item.sku}</Typography>
                </Box>
                <Typography variant="body2" fontWeight={700}>{fmt(item.total_price ?? item.quantity * item.unit_price)}</Typography>
              </Box>
            ))}
          </Stack>
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', pt: 1.5, borderTop: '2px solid #E2E8F0', mt: 1 }}>
            <Typography fontWeight={800} fontSize={15}>Total: {fmt(order.total_amount)}</Typography>
          </Box>
        </Box>
      </Box>
      ...
      {(canFulfill || canDeliver || canCancel) && (
        <Box sx={{ p: 2, borderTop: '1px solid #E2E8F0', display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          {canFulfill && (
            <Button onClick={() => { onClose(); onFulfill(order); }} variant="contained" color="success" size="small" startIcon={<ShipIcon />} sx={{ borderRadius: 2, flex: 1 }}>
              Fulfill & Dispatch
            </Button>
          )}
          {canDeliver && (
            <Button onClick={() => { onClose(); onDeliver(order); }} variant="contained" size="small" startIcon={<DoneIcon />} sx={{ borderRadius: 2, flex: 1 }}>
              Mark Delivered
            </Button>
          )}
          {canCancel && (
            <Button onClick={() => { onClose(); onCancel(order); }} variant="outlined" color="error" size="small" startIcon={<CancelIcon />} sx={{ borderRadius: 2, flex: 1 }}>
              Cancel Order
            </Button>
          )}
        </Box>
...
export default function SalesOrders() {
  ...
  useEffect(() => {
    warehouseAPI.getAll({ limit: 200 }).then((r) => {
      const all = r.data.data || [];
      setWarehouses(all);
      if (!isAdmin) setUserWHIds(all.filter((w) => w.manager_id === user?.id).map((w) => w.warehouse_id));
    }).catch(() => {});
  }, [isAdmin, user?.id]);
  ...
  useEffect(() => {
    ...
    debRef.current = setTimeout(() => { setDeb(search); setPage(0); }, 350);
  ...
  const fetchCounts = useCallback(async () => {
    try {
      ...
      const results = await Promise.all(statuses.map((s) => salesOrderAPI.getAll({ status: s, limit: 1 })));
      const counts = {};
      statuses.forEach((s, i) => { counts[s] = results[i].data.data.total; });
      setTabCounts(counts);
    } catch { /* silent */ }
  }, []);
  ...
  const resetFilters = () => {
    setTab('all'); setSearch(''); setDeb('');
    setWhFilter(''); setFrom(''); setTo(''); setPage(0);
  };
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1.5 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="outlined" size="small" startIcon={<RefreshIcon />} onClick={refresh}
            sx={{ borderRadius: 2, fontSize: 13, textTransform: 'none' }} disabled={loading}>
            Refresh
          </Button>
          {canCreate && (
      ...
      {/* ── Filters toolbar ── */}
      <Paper variant="outlined" sx={{ p: 1.5, borderRadius: 2, display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        <TextField
        ...
        {isAdmin && (
          <TextField select size="small" label="Warehouse" value={whFilter}
            ...
            {warehouses.map((w) => <MenuItem key={w.warehouse_id} value={w.warehouse_id}>{w.name}</MenuItem>)}
          </TextField>
        )}
              ...
              {!loading && orders.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={9} sx={{ textAlign: 'center', py: 6, color: '#94A3B8', borderBottom: 'none' }}>
                    ...
                    {hasFilters && (
                      <Button size="small" onClick={resetFilters} sx={{ mt: 1, textTransform: 'none', fontSize: 12 }}>
                        Clear filters
                      </Button>
                    )}
                  </TableCell>
      ...
      <CreateOrderModal
        ...
        onSuccess={(msg) => { showSnack(msg); setCreateOpen(false); refresh(); }}
      ...
      <OrderDrawer
        ...
        onFulfill={(o) => { setView(null); setFulfill(o); }}
        onCancel={(o) => { setView(null); setCancel(o); }}
