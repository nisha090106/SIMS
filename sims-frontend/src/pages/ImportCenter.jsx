      ...
      try {
        ...
        if (res.data && res.data.success) {
          ...
          if (job.status === 'completed' || job.status === 'failed') {
            setPolling(false);
            showToast(
              `Import job finished with status: ${job.status}`,
        ...
        <section className='upload-section-panel'>
          ...
          {activeTab === 'stock' && (
            <div className='stock-additional-form'>
              <label htmlFor='import-warehouse-selector'>
                Target Warehouse for Stock <span className='req-asterisk'>*</span>
              </label>
              <select
          ...
          >
            ...
            {selectedFile ? (
              ...
              <div className='prompt-text'>
                <p>
                  Drag and drop your CSV or Excel file here, or <span>click to browse</span>
                </p>
                      ...
                      style={{
                        width: `${
                          activeJob.total_rows > 0
                            ? Math.round(
                                ((activeJob.processed_rows + activeJob.failed_rows) /
                ...
                <div className='active-error-logs-box'>
                  ...
                  <div className='errors-scroller'>
                    {(() => {
                      try {
                        const parsed =
                          typeof activeJob.error_log === 'string'
                            ? JSON.parse(activeJob.error_log)
                            : activeJob.error_log;
                        if (!parsed || parsed.length === 0)
                          return <p className='no-errors'>No error summary available.</p>;
                        return parsed.map((err, idx) => {
                          const formattedErr = formatImportError(
                            err,
                            activeJob.job_type || 'stock',
                          );
                          return (
                  ...
                  return (
                    <React.Fragment key={job.id}>
                      ...
                      >
                        ...
                        <td>
                          <span className={`badge-status-history ${job.status}`}>{job.status}</span>
                        </td>
                        <td className='align-center font-bold'>{job.total_rows}</td>
                        <td className='align-center text-success font-semibold'>
                          {job.processed_rows}
                        </td>
                        <td className='align-center text-danger font-semibold'>
                          {job.failed_rows}
                        </td>
                        <td>{triggeredName}</td>
                            ...
                            <div className='history-expanded-logs-container'>
                              ...
                              {!job.error_log ? (
                                <p className='no-errors'>
                                  No errors logged. Import was fully successful.
                                </p>
                              ) : (
                                <div className='expanded-errors-grid'>
                                  <div className='meta-stats'>
                                    <span>
                                      Failed Rows: <strong>{job.failed_rows}</strong>
                                    </span>
                                    <span>
                                      Total File Rows: <strong>{job.total_rows}</strong>
                                    </span>
                                  </div>
                                  <div className='errors-list-box'>
                                    {(() => {
                                      try {
                                        const parsed =
                                          typeof job.error_log === 'string'
                                            ? JSON.parse(job.error_log)
                                            : job.error_log;
                                        if (!parsed || parsed.length === 0)
                                          return (
                                            <p className='no-errors'>No error summary available.</p>
                                          );
                                        return parsed.map((err, idx) => {
                                          const formattedErr = formatImportError(err, job.job_type);
                                          return (
                                            <div key={idx} className='error-log-item-row'>
                                              <strong className='row-num text-danger'>
                                                Row {err.row || idx + 1}:
                                              </strong>
