...
const schema = Yup.object({
  ...
  unit_price:    Yup.number().typeError('Must be a number').positive('Must be positive').required('Selling price is required'),
  cost_price:    Yup.number().typeError('Must be a number').min(0, 'Must be ≥ 0').nullable(),
  reorder_level: Yup.number().typeError('Must be a number').integer().min(0).required('Reorder level is required'),
...
export default function ProductForm({
  product, categories, onSuccess, onCancel, onCategoryCreated,
}) {
  ...
  const fieldErr = (name) =>
    F.touched[name] && F.errors[name] ? F.errors[name] : undefined;
    ...
    <>
      ...
      >
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 6 }}>
            ...
            >
              {F.values.image_url
                ? <img src={F.values.image_url} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ...
        <div>
          ...
          <select
            ...
            onFocus={(e) => {
              e.target.style.borderColor = fieldErr('category') ? 'var(--color-danger)' : 'var(--color-primary)';
              e.target.style.boxShadow = fieldErr('category')
                ? '0 0 0 3px rgba(220,38,38,0.12)' : '0 0 0 3px rgba(37,99,235,0.12)';
            }}
            onBlur={(e) => {
              F.handleBlur(e);
              e.target.style.borderColor = fieldErr('category') ? 'var(--color-danger)' : 'var(--color-border)';
          ...
          {fieldErr('category') && (
            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-danger)', display: 'block', marginTop: 4 }}>
              {fieldErr('category')}
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)', fontFamily: 'var(--font-sans)' }}>
            {F.values.is_active ? 'Product is visible and orderable' : 'Product is hidden from catalogue'}
          </span>
        </div>

        {/* Global error */}
        {F.errors.name && F.submitCount > 0 && !F.touched.name && (
          <p style={{ color: 'var(--color-danger)', fontSize: 'var(--text-sm)', fontFamily: 'var(--font-sans)', margin: 0 }}>
            {F.errors.name}
          </p>
        )}
      </form>
      ...
      <div style={{
        ...
        <Button
          ...
          size="sm"
          loading={F.isSubmitting && !addAnotherPending}
          onClick={() => { setAddAnotherPending(false); F.submitForm(); }}
        >
      ...
      <Modal
        open={newCatModal}
        onClose={() => { setNewCatModal(false); setNewCatName(''); setNewCatError(''); }}
      ...
      >
        ...
        <Modal.Footer>
          <Button variant="ghost" size="sm" onClick={() => { setNewCatModal(false); setNewCatName(''); setNewCatError(''); }}>
            Cancel
          </Button>
