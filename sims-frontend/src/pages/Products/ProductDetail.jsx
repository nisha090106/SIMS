...
function reducer(s, a) {
  switch (a.type) {
    ...
    case 'OK':      return { loading: false, error: null, product: a.product };
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button
          ...
          style={{
            ...
            gap: 4, fontSize: 'var(--text-sm)', fontFamily: 'var(--font-sans)',
            padding: 0,
          }}
        ...
        <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-primary)', fontWeight: 600, fontFamily: 'var(--font-sans)' }}>
          {product.name}
        </span>
      </div>
        ...
        <Card.Body>
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              {product.description && (
                <p style={{
                  ...
                  fontFamily: 'var(--font-sans)', lineHeight: 1.55, maxWidth: 560,
                }}>
                  {product.description}
                </p>
              )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 160 }}>
              <KpiPill label="Total Stock" value={totalStock} badge={<Badge variant={stockVar} size="sm" dot>{
                totalStock === 0 ? 'Out of Stock' : totalStock <= product.reorder_level ? 'Low Stock' : 'In Stock'
              }</Badge>} />
              <KpiPill label="Selling Price" value={fmt(product.unit_price)} />
              {product.cost_price && (
                <KpiPill label="Cost Price" value={fmt(product.cost_price)} />
              )}
        ...
        {TABS.map(({ id: tid, label, icon: Icon }) => (
          <button
            ...
            style={{
              background: 'none', border: 'none', cursor: 'pointer',
              padding: '10px 18px',
              fontSize: 'var(--text-sm)', fontWeight: tab === tid ? 700 : 500,
...
function OverviewTab({ product }) {
  const fields = [
    { label: 'SKU',            value: <code style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)' }}>{product.sku}</code> },
    ...
    { label: 'Status',         value: product.is_active ? <Badge variant="success">Active</Badge> : <Badge variant="neutral">Inactive</Badge> },
      ...
      <Card.Body>
          ...
          {fields.map(({ label, value }) => (
            <div key={label}>
              <dt style={{
                ...
                letterSpacing: '0.06em', fontFamily: 'var(--font-sans)', marginBottom: 4,
              }}>
                {label}
              </dt>
              <dd style={{
        ...
        {product.description && (
          <>
            ...
            <div>
              <p style={{
                margin: '0 0 6px', fontSize: 'var(--text-xs)', fontWeight: 700,
                color: 'var(--color-text-muted)', textTransform: 'uppercase',
                letterSpacing: '0.06em', fontFamily: 'var(--font-sans)',
              }}>Description</p>
              <p style={{
                margin: 0, fontSize: 'var(--text-base)', lineHeight: 1.65,
      ...
      <Table
        ...
        renderRow={(inv, i) => {
          ...
          return (
            <tr key={inv.id ?? i}>
              <td style={tdStyle}>
                <div style={{ fontWeight: 600 }}>{inv.warehouse?.name ?? `Warehouse #${inv.warehouse_id}`}</div>
              ...
              <td style={{ ...tdStyle, textAlign: 'center' }}>
                <Badge variant={sVar} size="sm">{qty}</Badge>
              </td>
              ...
              <td style={{ ...tdStyle, fontSize: 'var(--text-sm)' }}>
                {inv.expiry_date
                  ? <span style={{ color: new Date(inv.expiry_date) < new Date() ? 'var(--color-danger)' : 'inherit' }}>
                      {fmtDate(inv.expiry_date)}
                    </span>
                  : '—'}
              </td>
              <td style={{ ...tdStyle, textAlign: 'center' }}>
                <Badge variant={sVar} size="sm" dot>{sLabel}</Badge>
              </td>
              <td style={{ ...tdStyle, textAlign: 'right', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>
...
function PurchasesTab({ product }) {
  ...
  return (
    <Card title="Purchase Order History" subtitle="Last 10 purchase orders containing this product" padding={false}>
      <Table
        columns={columns}
        data={history}
        loading={false}
        ...
        renderRow={(po, i) => (
          <tr key={po.po_id ?? i}>
            <td style={tdStyle}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--color-primary)' }}>
                {po.po_number}
              </span>
            </td>
            <td style={tdStyle}>{po.supplier_name}</td>
            <td style={{ ...tdStyle, color: 'var(--color-text-secondary)', fontSize: 'var(--text-sm)' }}>
...
function KpiPill({ label, value, badge }) {
  return (
    <div style={{
      ...
      {badge
        ? badge
        : <span style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)' }}>
            {value ?? '—'}
          </span>
      }
    </div>
