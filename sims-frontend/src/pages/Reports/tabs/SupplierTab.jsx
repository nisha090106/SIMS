...
export default function SupplierTab() {
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'right',
      render: (v) => (
        <span style={{ color: parseFloat(v) > 20 ? '#f43f5e' : '#64748b' }}>
    ...
    <>
      ...
      <div className='reports-kpi-grid'>
        <KpiCard
          ...
          color='blue'
          loading={loading}
          value={fmtNum(data?.summary?.totalSuppliers)}
        />
        <KpiCard
          ...
          color='green'
          loading={loading}
          value={fmtNum(data?.summary?.activeSuppliers)}
        />
        <KpiCard
          ...
          color='purple'
          loading={loading}
          value={fmtCurrency(data?.summary?.totalSpent)}
        />
        <KpiCard
          ...
          color='orange'
          loading={loading}
          value={fmtNum(data?.summary?.totalOrders)}
        />
        <KpiCard
          icon={<Star size={20} />}
          ...
          <div className='reports-chart-card'>
            ...
            <ResponsiveContainer width='100%' height={280}>
              <BarChart data={top6} margin={{ left: -10 }}>
                ...
                <Tooltip
                  {...TOOLTIP_STYLE}
                  formatter={(v) => [`${parseFloat(v).toFixed(1)}%`, 'Fulfillment']}
                />
                <Bar dataKey='fulfillmentRate' name='Fulfillment %' radius={[4, 4, 0, 0]}>
                  {top6.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={
                        parseFloat(entry.fulfillmentRate) >= 80
                          ? '#10b981'
                          : parseFloat(entry.fulfillmentRate) >= 50
                            ? '#f59e0b'
                            : '#f43f5e'
                      }
                    />
