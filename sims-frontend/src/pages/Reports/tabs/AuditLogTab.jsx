...
import { downloadBlob, fmtNum, fmtDateTime, STATUS_PILL, CHART_COLORS, TOOLTIP_STYLE } from '../reportUtils';
...
export default function AuditLogTab() {
  ...
  const [filters, setFilters] = useState({
    userId: '', action: '', from: '', to: '', limit: 25,
  });

  useEffect(() => {
    settingsAPI.getUsers({ limit: 200 })
  ...
  const columns = [
    ...
    { key: 'role',      label: 'Role',      render: v => v?.toUpperCase() },
    {
      ...
      render: v => (
        <span className={`status-pill ${STATUS_PILL[v?.toLowerCase()] ?? 'draft'}`} style={{ textTransform: 'none' }}>
          {v?.replace(/_/g, ' ')}
        </span>
      )
    },
    ...
    {
      key: 'changes',   label: 'Changes/Details',
      render: v => v ? (
        <div style={{ maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'monospace', fontSize: 11 }} title={v}>
          {v}
        </div>
      ) : '—'
    },
    ...
    <>
      <FilterBar onApply={handleApply} onExport={handleExport} loading={loading}>
        <FilterField label="User">
          <select value={filters.userId} onChange={e => setFilters(p => ({ ...p, userId: e.target.value }))}>
      ...
      <div className="reports-kpi-grid">
        <KpiCard icon={<History size={20}/>}  label="Total Logs"    color="blue"   loading={loading} value={fmtNum(data?.summary?.totalLogs)} />
        <KpiCard icon={<Users size={20}/>}    label="Unique Users"  color="purple" loading={loading} value={fmtNum(data?.summary?.uniqueUsers)} />
        <KpiCard icon={<Activity size={20}/>} label="Action Types"  color="green"  loading={loading} value={fmtNum(actionChart.length)} />
      </div>
