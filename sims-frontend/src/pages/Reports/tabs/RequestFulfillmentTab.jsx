...
export default function RequestFulfillmentTab({ userRole }) {
  ...
  const columns = [
    ...
    { key: 'totalFulfilled', label: 'Fulfilled', align: 'right', render: (v) => fmtNum(v) },
    {
      ...
      render: (v) => (
        <span
          style={{
            fontWeight: 700,
            color: parseFloat(v) >= 80 ? '#10b981' : parseFloat(v) >= 50 ? '#f59e0b' : '#f43f5e',
          }}
        >
    ...
    },
    {
      ...
      render: (v) => (
        <span
          className={`status-pill ${v === 'high' || v === 'urgent' ? 'cancelled' : v === 'medium' ? 'pending' : 'approved'}`}
    ...
    <>
      ...
      <div className='reports-kpi-grid'>
        <KpiCard
          ...
          color='blue'
          loading={loading}
          value={fmtNum(data?.summary?.totalRequests)}
        />
        <KpiCard
          ...
          loading={loading}
          value={
            data?.summary?.overallFulfillmentRate ? `${data.summary.overallFulfillmentRate}%` : '—'
          }
        />
        <KpiCard
          ...
          value={fmtNum(
            data?.summary?.statusBreakdown?.fulfilled ||
              data?.summary?.statusBreakdown?.fulfilled_qty,
          )}
        />
        <KpiCard
          icon={<Clock size={20} />}
          ...
          color='orange'
          loading={loading}
          value={fmtNum(data?.summary?.statusBreakdown?.pending)}
        />
      </div>
