...
function reducer(s, a) {
  switch (a.type) {
    ...
    case 'OK':
      return { loading: false, error: null, po: a.po };
    case 'ERROR':
...
function StatusStepper({ status }) {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'center', gap: 0, overflowX: 'auto', padding: '4px 0' }}
    >
      {steps.map((step, i) => {
                ...
                style={{
                  ...
                  background:
                    i < activeIdx && !isCancelled
                      ? stepLabels[steps[i + 1]]?.color || 'var(--color-border)'
                      : 'var(--color-border)',
                  margin: '0 2px',
                  marginBottom: 18,
                }}
      ...
      {isCancelled && (
        <Badge variant='danger' size='sm' dot style={{ marginLeft: 12 }}>
          Cancelled
        </Badge>
      )}
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                {po.auto_drafted && (
                  <Badge variant='warning' size='sm'>
                    <AutoIcon style={{ fontSize: 11 }} /> Auto-drafted
                  </Badge>
                )}
              </div>
          ...
          >
            ...
            {(status === 'draft' || status === 'submitted') && isAdmin && (
              <>
                <Button
                  variant='primary'
                  size='sm'
                  leftIcon={<ApproveIcon style={{ fontSize: 16 }} />}
                ...
                <Button
                  ...
                  size='sm'
                  leftIcon={<CancelIcon style={{ fontSize: 16 }} />}
            ...
            {status === 'approved' && (isAdmin || isMgr) && (
              <Button
                variant='primary'
                size='sm'
                leftIcon={<ShipIcon style={{ fontSize: 16 }} />}
        ...
        <Card title='Supplier Information'>
          <Card.Body>
            ...
            >
              ...
              <DLItem
                label='Lead Time'
                value={po.supplier.lead_time ? `${po.supplier.lead_time} days` : '—'}
              />
            </dl>
          </Card.Body>
        </Card>
      ...
      <Card title='Line Items' padding={false}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td
                    colSpan={po.status === 'received' ? 7 : 6}
                    style={{
                      ...tdStyle,
                      textAlign: 'center',
                      color: 'var(--color-text-muted)',
                ...
                items.map((item, idx) => (
                  <tr key={idx}>
                    <td style={{ ...tdStyle, color: 'var(--color-text-muted)', width: 40 }}>
                      {idx + 1}
                    </td>
                    <td style={tdStyle}>
        ...
        >
          <div style={{ minWidth: 220, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div
              style={{ borderTop: '2px solid var(--color-border)', paddingTop: 6, marginTop: 2 }}
            >
      ...
      >
        <Modal.Body>
          ...
          >
            Are you sure you want to approve this purchase order? This will move it to the Confirmed
            status.
          </p>
        </Modal.Body>
        <Modal.Footer>
      ...
      >
        <Modal.Body>
          <div
            style={{ display: 'flex', gap: 14, alignItems: 'flex-start', flexDirection: 'column' }}
          >
...
function TotalRow({ label, value, bold = false }) {
  return (
    ...
    >
      ...
      <span
        style={{
          fontSize: bold ? 'var(--text-lg)' : 'var(--text-sm)',
          fontFamily: 'var(--font-mono)',
