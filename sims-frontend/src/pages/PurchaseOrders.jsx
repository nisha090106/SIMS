...
export default function PurchaseOrders() {
  ...
  const handleApprovePO = async (poId) => {
    try {
      ...
      setPos(
        pos.map((p) => (p.purchase_order_id === poId ? response.data.po || response.data : p)),
      );
  ...
  const handleReceiveGoods = async () => {
    ...
    try {
      const response = await api.post(
        `/purchase-orders/${selectedPO.purchase_order_id}/receive`,
        receiveData,
      );
      setPos(
        pos.map((p) =>
          p.purchase_order_id === selectedPO.purchase_order_id
            ? response.data.po || response.data
            : p,
        ),
      );
            ...
            {pos.length > 0 ? (
              pos.map((po) => (
                <tr key={po.purchase_order_id}>
                  ...
                  <td>
                    <span className='amount'>${parseFloat(po.total_amount).toFixed(2)}</span>
                  </td>
                  <td>
              ...
              {createStep === 3 && (
                <div style={{ lineHeight: '1.8' }}>
                  <p>
                    ...
                    {suppliers.find((s) => s.supplier_id == formData.supplier_id)?.name}
                  </p>
                  <p>
        ...
        <div className='modal-overlay' onClick={() => setShowDetailModal(false)}>
          <div className='modal-content modal-large' onClick={(e) => e.stopPropagation()}>
            ...
            <div className='modal-body'>
              ...
              <p>
                <strong>Order Date:</strong> {new Date(selectedPO.created_at).toLocaleDateString()}
              </p>
              <p>
