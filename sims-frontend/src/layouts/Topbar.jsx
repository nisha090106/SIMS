    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 700, fontSize: 14, fontFamily: 'var(--font-sans)', color: 'var(--color-text-primary)' }}>
            Notifications
          </span>
          {unreadCount > 0 && (
            <span style={{
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <ActionBtn title="View all" onClick={() => { navigate('/notifications'); onClose(); }}>
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {loading && recent.length === 0 ? (
          ...
          <div style={{ padding: '40px 16px', textAlign: 'center' }}>
            <BellIcon style={{ fontSize: 36, color: 'var(--color-border)', display: 'block', margin: '0 auto 8px' }} />
            <p style={{ margin: 0, color: 'var(--color-text-muted)', fontSize: 13, fontFamily: 'var(--font-sans)' }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: hovered ? 'var(--color-surface-alt)' : (!n.is_read ? 'rgba(37,99,235,0.03)' : 'transparent'),
        borderBottom: '1px solid var(--color-border)',
        position: 'relative',
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{
          margin: 0, fontSize: 12, fontWeight: n.is_read ? 500 : 700,
        ...
        <p style={{
          margin: '2px 0 0', fontSize: 12, color: 'var(--color-text-muted)',
...
function ActionBtn({ title, onClick, children }) {
  return (
    <button
      ...
      onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-surface-alt)'; e.currentTarget.style.color = 'var(--color-text-primary)'; }}
  ...
  return (
    <header style={{
      ...
      <button onClick={onToggle} aria-label="Toggle sidebar"
        style={iconBtnStyle} onMouseEnter={iconBtnHover} onMouseLeave={iconBtnLeave}>
        ...
        {crumbs.map((crumb, i) => (
          <React.Fragment key={i}>
            ...
            <span style={{
              ...
              color: i === crumbs.length - 1 ? 'var(--color-text-primary)' : 'var(--color-text-muted)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
        {/* Search */}
        <button aria-label="Search" style={iconBtnStyle} onMouseEnter={iconBtnHover} onMouseLeave={iconBtnLeave}>
        ...
        <div ref={dropdownRef} style={{ position: 'relative', marginLeft: 8 }}>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', lineHeight: 1 }}>
              <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)', whiteSpace: 'nowrap' }}>
                {fullName.split(' ')[0]}
              </span>
              <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', fontFamily: 'var(--font-sans)', marginTop: 2, textTransform: 'capitalize' }}>
                {role}
              </span>
            </div>
            <span style={{ fontSize: 10, color: 'var(--color-text-muted)', transform: dropdownOpen ? 'rotate(180deg)' : 'none', transition: 'transform var(--transition-base)', lineHeight: 1 }}>▼</span>
          </button>
          ...
          {dropdownOpen && (
            <div style={{
              ...
              width: 220, padding: 6, zIndex: 1200,
              animation: 'topbar-dd-in 0.15s ease',
            }}>
              <div style={{ padding: '10px 12px 8px', borderBottom: '1px solid var(--color-border)', marginBottom: 4 }}>
                <div style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{fullName}</div>
                <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', fontFamily: 'var(--font-sans)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 1 }}>{email}</div>
              ...
              <DropdownItem icon={<ProfileIcon style={{ fontSize: 16 }} />} label="Profile" onClick={() => { setDropdownOpen(false); navigate('/settings'); }} />
              <DropdownItem icon={<SettingsIcon style={{ fontSize: 16 }} />} label="Settings" onClick={() => { setDropdownOpen(false); navigate('/settings'); }} />
              <div style={{ height: 1, background: 'var(--color-border)', margin: '4px 0' }} />
              <DropdownItem icon={<LogoutIcon style={{ fontSize: 16 }} />} label="Sign Out" danger onClick={handleLogout} />
            </div>
          )}
        </div>
      </div>
...
const iconBtnHover = (e) => { e.currentTarget.style.background = 'var(--color-surface-alt)'; e.currentTarget.style.color = 'var(--color-text-primary)'; };
const iconBtnLeave = (e) => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = 'var(--color-text-secondary)'; };
...
const DropdownItem = ({ icon, label, onClick, danger = false }) => (
  <button onClick={onClick} style={{
    ...
    fontWeight: 500, color: danger ? 'var(--color-danger)' : 'var(--color-text-primary)',
    cursor: 'pointer', textAlign: 'left', transition: 'background var(--transition-base)',
  }}
    onMouseEnter={e => e.currentTarget.style.background = danger ? 'var(--color-danger-soft)' : 'var(--color-surface-alt)'}
    onMouseLeave={e => e.currentTarget.style.background = 'none'}
  >
    <span style={{ color: danger ? 'var(--color-danger)' : 'var(--color-text-secondary)', display: 'flex' }}>{icon}</span>
    {label}
  </button>
);
